 ...
 * 
 * 使用方法:
 * 将此脚本通过Nginx等反向代理注入到Dify前端的 index.html 文件 <body> 标签底部。
 */
...
(function() {
  try {
    ...
    if (accessToken || refreshToken) {
      console.log('[Dify SSO] Detected SSO tokens in URL.');

      if (accessToken) {
        // 存储access_token到localStorage
        localStorage.setItem('console_token', accessToken);
        console.log('[Dify SSO] Access token stored successfully.');
      }

      if (refreshToken) {
        // 存储refresh_token到localStorage
        localStorage.setItem('refresh_token', refreshToken);
        console.log('[Dify SSO] Refresh token stored successfully.');
      }
      ...
      console.log('[Dify SSO] URL cleaned. Reloading page to apply login state...');
